...
filename1 = 'dados_juntos_laura_jat.txt';
filename2 = 'dados_juntos_josy_jat.txt';
filename3 = 'dados_juntos_gabi_jat.txt';
...
save('mediasionosfericasJAT.mat', 'foF2', 'hF','hmF2');
...
% Replicação segura das médias e desvios
numDias = ceil(length(full_time_numeric)/length(mediahF));
...
% Replicação segura das médias e desvios
numDias = ceil(length(full_time_numeric)/length(mediahmF2));
...
numDias = ceil(length(full_time_numeric)/length(mediaf0F2));
...
% Ajustar média e desvio para ter o mesmo tamanho do vetor de tempo
numDias_hF = ceil(length(full_time_numeric)/length(mediahF));
mediahF  = repmat(mediahF(:), numDias_hF, 1);
desviohF = repmat(desviohF(:), numDias_hF, 1);
mediahF  = mediahF(1:length(full_time_numeric));
desviohF = desviohF(1:length(full_time_numeric));

numDias_hmF2 = ceil(length(full_time_numeric)/length(mediahmF2));
mediahmF2  = repmat(mediahmF2(:), numDias_hmF2, 1);
desviohmF2 = repmat(desviohmF2(:), numDias_hmF2, 1);
mediahmF2  = mediahmF2(1:length(full_time_numeric));
desviohmF2 = desviohmF2(1:length(full_time_numeric));

numDias_foF2 = ceil(length(full_time_numeric)/length(mediaf0F2));
mediaf0F2  = repmat(mediaf0F2(:), numDias_foF2, 1);
desviof0F2 = repmat(desviof0F2(:), numDias_foF2, 1);
mediaf0F2  = mediaf0F2(1:length(full_time_numeric));
desviof0F2 = desviof0F2(1:length(full_time_numeric));
%}
