...
filename1 = 'dados_juntos_laura_arg.txt';
filename2 = 'dados_juntos_josy_arg.txt';
filename3 = 'dados_juntos_gabi_arg.txt';
...
save('mediasionosfericasARG.mat', 'foF2', 'hF','hmF2');
...
time2 = linspace(full_time_numeric(1), full_time_numeric(end), numel(SymH));
xlim([full_time_numeric(1) full_time_numeric(end)]);  % mesmo range dos dados ionosféricos
...
% Descobrir quantas vezes repetir
numDias = length(full_time_numeric) / length(mediahF);

% Arredonda para evitar problema de números quebrados
numDias = round(numDias);

% Repete exatamente para preencher todo o vetor
mediahF  = repmat(mediahF(:), numDias, 1);
desviohF = repmat(desviohF(:), numDias, 1);

% Ajusta para ter exatamente o mesmo tamanho
mediahF  = mediahF(1:length(full_time_numeric));
desviohF = desviohF(1:length(full_time_numeric));
...
% Descobrir quantas vezes repetir
numDias = length(full_time_numeric) / length(mediahmF2);

% Arredondar para evitar número quebrado
numDias = round(numDias);

% Repete para preencher todo o vetor
mediahmF2  = repmat(mediahmF2(:), numDias, 1);
desviohmF2 = repmat(desviohmF2(:), numDias, 1);

% Ajustar para ter o mesmo tamanho
mediahmF2  = mediahmF2(1:length(full_time_numeric));
desviohmF2 = desviohmF2(1:length(full_time_numeric));
...
% Ajustar média e desvio para todos os dias concatenados
numDias = length(full_time_numeric) / length(mediaf0F2);
numDias = round(numDias);
mediaf0F2  = repmat(mediaf0F2(:), numDias, 1);
desviof0F2 = repmat(desviof0F2(:), numDias, 1);

% Garantir mesmo tamanho do vetor de tempo
mediaf0F2  = mediaf0F2(1:length(full_time_numeric));
desviof0F2 = desviof0F2(1:length(full_time_numeric));
...
Garantir que tenham exatamente o mesmo tamanho de full_time_numeric
mediahF  = mediahF(1:length(full_time_numeric));
desviohF = desviohF(1:length(full_time_numeric));


Ajustar médias e desvios para ?hmF2
numDias = length(full_time_numeric) / length(mediahmF2);
numDias = round(numDias);
...
Ajustar médias e desvios para ?foF2
numDias = length(full_time_numeric) / length(mediaf0F2);
numDias = round(numDias);
...
ax.YColor = 'k';
hold on;
% yline(mean(diff_hF, 'omitnan'), 'Color',grayColor,  'LineWidth', 2);
% yline(mean(diff_hF, 'omitnan') + std(diff_hF, 'omitnan'),'--', 'Color',grayColor, 'LineWidth', 1);
% yline(mean(diff_hF, 'omitnan') - std(diff_hF, 'omitnan'),'--',  'Color',grayColor, 'LineWidth', 1);
ylabel('\Delta h''F');
set(gca, 'XTickLabel', []); % Remove x-tick labels
% Set major ticks larger and custom labels with days
...
%diff_hF = fillmissing(diff_hF, 'linear');  % Interpolação linear
...
% diff_hF(nan_mask_hF) = NaN;  % Descomente esta linha se quiser manter NaN
...
ylim([-200,200])
hold on;
% yline(mean(diff_hmF2, 'omitnan'),  'Color',grayColor, 'LineWidth', 2);
% yline(mean(diff_hmF2, 'omitnan') + std(diff_hmF2, 'omitnan'), '--',  'Color',grayColor, 'LineWidth', 1);
% yline(mean(diff_hmF2, 'omitnan') - std(diff_hmF2, 'omitnan'), '--', 'Color',grayColor, 'LineWidth', 1);
ylabel('\Delta hpF2');
set(gca, 'XTickLabel', []); % Remove x-tick labels
% Set minor ticks on the x-axis
...
ylim([-5,5])
hold on;
% yline(mean(diff_f0F2, 'omitnan'), 'Color',grayColor, 'LineWidth', 2);
% yline(mean(diff_f0F2, 'omitnan') + std(diff_f0F2, 'omitnan'), '--','Color',grayColor, 'LineWidth', 1);
% yline(mean(diff_f0F2, 'omitnan') - std(diff_f0F2, 'omitnan'), '--','Color',grayColor, 'LineWidth', 1);
ylabel('\Delta foF2');
xlabel('Time (DD/Aug/2017)');
% Set minor ticks on the x-axis
ax = gca;
ax.YColor = 'k';
...
month_day = arrayfun(@(x) datestr(x, 'dd'), dates(1:end-1), 'UniformOutput', false);
month_day{end+1} = '31';  % Adiciona o último dia manualmente
month_day{end+2} = '09/01';  % Adiciona o último dia manualmente
ax.XTickLabel = month_day;

% Create rectangle
annotation(figure1,'rectangle',...
    [0.155377897755002 0.111353711790393 0.0245208333333333 0.810807736448835],...
...
% Create rectangle
annotation(figure1,'rectangle',...
    [0.330166666666666 0.11214953271028 0.0245208333333333 0.809247847202384],...
    'Color',[0.0745098039215686 0.623529411764706 1],...
    'FaceColor',[0.0745098039215686 0.623529411764706 1],...
    'FaceAlpha',0.3);
...
% Create rectangle
annotation(figure1,'rectangle',...
    [0.480166666666666 0.11214953271028 0.0245208333333333 0.809247847202384],...
    'Color',[0.0745098039215686 0.623529411764706 1],...
    'FaceColor',[0.0745098039215686 0.623529411764706 1],...
    'FaceAlpha',0.3);
...
% Create rectangle
annotation(figure1,'rectangle',...
    [0.530166666666665 0.11214953271028 0.0245208333333333 0.809247847202384],...
    'Color',[1 0 0],...
    'FaceColor',[1 0 0],...
    'FaceAlpha',0.3);
...
% Create rectangle
annotation(figure1,'rectangle',...
    [0.605687499999999 0.11214953271028 0.0245208333333333 0.810339550259152],...
    'Color',[1 0 0],...
    'FaceColor',[1 0 0],...
    'FaceAlpha',0.3);
...
% Create rectangle
annotation(figure1,'rectangle',...
    [0.579645833333333 0.11214953271028 0.0245208333333333 0.810339550259152],...
    'Color',[1 0 0],...
    'FaceColor',[1 0 0],...
    'FaceAlpha',0.3);
...
% Create rectangle
annotation(figure1,'rectangle',...
    [0.680166666666666 0.11214953271028 0.0245208333333333 0.810339550259152],...
    'Color',[1 0 0],...
    'FaceColor',[1 0 0],...
    'FaceAlpha',0.3);
...
% Create rectangle
annotation(figure1,'rectangle',...
    [0.804645833333333 0.113241235767049 0.0245208333333333 0.808156144145615],...
    'Color',[0.0745098039215686 0.623529411764706 1],...
    'FaceColor',[0.0745098039215686 0.623529411764706 1],...
    'FaceAlpha',0.3);
...
% Create rectangle
annotation(figure1,'rectangle',...
    [0.854124999999999 0.11214953271028 0.0245208333333333 0.809247847202384],...
    'Color',[0.0745098039215686 0.623529411764706 1],...
    'FaceColor',[0.0745098039215686 0.623529411764706 1],...
    'FaceAlpha',0.3);
...
% Create rectangle
annotation(figure1,'rectangle',...
    [0.879645833333333 0.113241235767049 0.0245208333333333 0.809247847202384],...
    'Color',[1 0 0],...
    'FaceColor',[1 0 0],...
    'FaceAlpha',0.3);
